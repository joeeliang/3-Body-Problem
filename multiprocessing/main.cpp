...
public:
    ...
    std::vector<long double> integrate(long double dt, int num_steps, bool save_positions = false) {
        std::vector<std::vector<std::array<long double, 2>>> positions;
        std::vector<long double> delta_energy;

        if (save_positions) {
            positions.resize(num_steps, std::vector<std::array<long double, 2>>(bodies.size()));
